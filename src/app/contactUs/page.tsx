      ...
      <div className="mx-auto mb-3 max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20 rounded-[3px]">
        <div className="mb-3 max-w-3xl text-center sm:text-center md:mx-auto md:mb-12">
          ...
          <h2 className="font-heading mb-4 font-bold tracking-tight text-4xl sm:text-5xl">
            {language === 'en' ? 'Get in Touch' : 'Liên hệ với chúng tôi'}
          </h2>
          <h3 className="mx-auto mt-4 max-w-3xl text-xl mb-8">
            {language === 'en' ? 'We look forward to hearing from you' : 'Chúng tôi mong nhận được phản hồi từ bạn'}
          </h3>
        </div>
          ...
          <div className="grid md:grid-cols-2">
            <div className="h-full pr-3">
              <p className="mt-3 mb-12 text-lg ">
                {language === 'en' ? 'We welcome your inquiries and feedback.' : 'Chúng tôi hoan nghênh ý kiến và phản hồi của bạn.'}
              </p>
              <ul className="mb-6 md:mb-0">
                <li className="flex">
                  ...
                  <div className="ml-4 mb-4">
                    <h3 className="mb-2 text-lg font-medium leading-6 ">
                      {language === 'en' ? 'Our Address' : 'Địa chỉ của chúng tôi'}
                    </h3>
                ...
                <li className="flex">
                  ...
                  <div className="ml-4 mb-4">
                    <h3 className="mb-2 text-lg font-medium leading-6 ">
                      {language === 'en' ? 'Working Hours' : 'Giờ làm việc'}
                    </h3>
            ...
            <div className="card h-fit max-w-6xl p-5 md:p-12 shadow-lg rounded-lg" id="form">
              <h2 className="mb-6 text-3xl font-bold text-black text-center">
                {language === 'en' ? 'Ready to Get Started?' : 'Sẵn sàng bắt đầu chưa?'}
              </h2>
              <form ref={form} onSubmit={sendEmail} className="space-y-10">
                <div className="mb-6">
                  <TextField
                    id="name"
                    label={language === 'en' ? 'Your Name' : 'Tên của bạn'}
                    variant="outlined"
                    fullWidth
                    name="name"
                  ...
                  <TextField
                    id="email"
                    label={language === 'en' ? 'Your Email Address' : 'Địa chỉ email của bạn'}
                    variant="outlined"
                    fullWidth
                    name="email"
                    style={{ marginBottom: '15px' }}
                  ...
                  <TextField
                    id="message"
                    label={language === 'en' ? 'Write Your Message...' : 'Viết tin nhắn của bạn...'}
                    variant="outlined"
                    fullWidth
                    multiline
                    rows={3}
                ...
                <div className="text-center">
                  ...
                  >
                    {loading ? <CircularProgress size={24} style={{ color: 'white' }} /> : (language === 'en' ? 'Send Message' : 'Gửi tin nhắn')}
                  </Button>
                </div>
                {status && (
